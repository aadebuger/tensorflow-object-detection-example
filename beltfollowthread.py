...
import os
import threading
videoports=os.getenv("video")
if videoports is None:
    print('videoport none')
    exit(-1)
videoport = int(videoports)
boxv=[]
detectworking=True
import arrow
...
def runvideo(n_frames=500, width=1280, height=720, with_threading=False):
        ...
        if with_threading:
            cap = VideoCaptureThreading(videoport)
        else:
            cap = cv2.VideoCapture(videoport)
...
#            cv2.waitKey(1) & 0xFF
            ...
            end1= time.time()
            print("frametime=",frametime,"endtime=",end1)
...
runvideo(10000000,640,480,True)
print("capture end")
